...
function httpGet(data){
    axios.get(data.url).then(function (res) {
        if(res.data.code === 1){
            ...
            data.fail && data.fail(res.data)
        }else if(res.data.code === -1){
            showMsgAxios(res.data.content,"error",2000)
            ...
            showMsgAxios("权限不足，请重新登录","error",2000)
        }
        data.complete && data.complete(res.data);
    }).catch(function (e) {
        showMsgAxios("系统繁忙，请重试","error",2000)
    })
}

function httpPost(data){
    axios.post(data.url,data.data).then(function (res) {
        if(res.data.code === 1){
            data.success && data.success(res.data)
        }else if(res.data.code === 0){
            data.fail && data.fail(res.data)
...
function showMsgAxios(message,type,time) {
    let str = ''
    switch (type) {
        case 'success':
